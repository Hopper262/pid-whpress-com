...
my $usage = "Usage: $0 [full11|full20|demo20|demoA1] <images-dir> <output-dir>\n";

our $version = $ARGV[0] || 'full20';
die $usage unless $version =~ /^(full11|full20|demo20|demoA1)$/;

our $imgdir = $ARGV[1];
die $usage unless -d $imgdir;

our $mapdir = $ARGV[2];
die $usage unless -d $mapdir;
...
while (1)
{
  ...
  my $err = $base->Write(sprintf("$mapdir/level%03d.png", $level));
  die $err if $err;
}
